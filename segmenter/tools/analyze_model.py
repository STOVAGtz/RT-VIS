...
def do_flop(cfg):
    ...
    for idx, data in zip(tqdm.trange(args.num_inputs), data_loader):  # noqa
        '''
        ## print inference data size
        import torch
        if idx < 10: # print 10 data info
            print(idx,type(data)) # data : list
            for sth in data:
                print("    ",type(sth)) # sth : dict
                for k in sth.keys():
                    if isinstance(sth[k],torch.Tensor):
                        print("        ",k,type(sth[k]),sth[k].shape)
                    else:
                        print("        ",k,type(sth[k]),sth[k])
        ##
        '''
