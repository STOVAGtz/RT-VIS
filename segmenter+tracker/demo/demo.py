...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        if args.output:
            ###
            #for path like 'datasets/ytvis_2019/valid/JPEGImages/1a894a8f98/*.jpg'
            out_foldername = os.path.join(args.output, args.input[0].split('/')[-2])
            ...
            if args.save_frames:
                for path, _vis_output in zip(args.input, visualized_output):
                    ...
                    ###
                    out_filename = os.path.join(out_foldername,os.path.basename(path))
                    '''
            ...
            out = cv2.VideoWriter(os.path.join(out_foldername, "visualization.mp4"), fourcc, 10.0, (W, H), True)
