...
class RepVggBlock(nn.Module):
    ...
    def forward(self, x):
        if (not hasattr(self, 'conv')) and (self.training == False):
            self.convert_to_deploy()
            assert hasattr(self,'conv'),"no attr 'conv' in RepVGG block"
...
class PyramidPoolingModuleFPN(nn.Module):
    @configurable
    def __init__(
            self,
        ...
        ### concat test (begin)
        final_concat = nn.Conv2d(3*256,256,kernel_size=1, bias=False)
        #self.add_module("final_concat_0", final_concat)
        ...
        for _ in range(len(self.in_channels) - 1, 0, -1):
            self.lateral_convs.append(ConvNormLayer(self.hidden_dim, self.hidden_dim, 1, 1, act=self.act))
            self.fpn_blocks.append(
                CSPRepLayer(self.hidden_dim * 2, self.hidden_dim, round(3 * self.depth_mult), act=self.act, expansion=self.expansion)
            )
        ...
        for _ in range(len(self.in_channels) - 1):
            self.downsample_convs.append(
                ConvNormLayer(self.hidden_dim, self.hidden_dim, 3, 2, act=self.act)
            )
    ...
    def forward_features(self, feats):
        ...
        outs = [inner_outs[0]]
        concat_list = []
        concat_list.append(inner_outs[0]) # max feature map
        for idx in range(len(self.in_channels) - 1):
            ...
            if idx==0:
                concat_list.append(F.interpolate(out, scale_factor=2., mode='bilinear'))
            if idx==1:
                concat_list.append(F.interpolate(out, scale_factor=4., mode='bilinear'))
            outs.append(out)

        outs[0] = self.final_concat(torch.cat(concat_list,1))
