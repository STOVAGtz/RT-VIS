...
class FastInstHead(nn.Module):
    ...
    def forward(self, features, targets=None,isfirst=False): # isfirst=True 视频第一帧
        return self.layers(features, targets,isfirst)

    def layers(self, features, targets=None,isfirst=False):
        mask_features, multi_scale_features = self.pixel_decoder.forward_features(features)
        predictions = self.predictor(multi_scale_features, mask_features, targets,isfirst)
        return predictions
