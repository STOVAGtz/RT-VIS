...
class Tracklet(object):
    ...
    def update(self, score, logit, mask, reid_embed, frame_id):
        ...
        if self.exist_frames == 0:
            ...
            all_reid_embed = torch.stack(self.reid_embeds[:-1], dim=0) #增加frame_id维度，shape[frame_id,q_num,vector_len]
            similarity = torch.sum(torch.einsum("bc,c->b", ## 余弦相似度：b个c维向量与c维向量相乘，求和，平均 == 向量依次点乘结果求平均
                                                F.normalize(all_reid_embed, dim=-1), # /norm2(vector)
                                                F.normalize(reid_embed, dim=-1))) / (len(self.reid_embeds) - 1)
            beta = max(0, similarity) # 使用相似度进行动量平均
...
class MemoryBank:
    ...
    def clean_dead_tracklets(self, cur_frame_id):
        # 删除已经死亡的 tracklets
        dead_ids = []
        for instance_id, tracklet in self.tracklets.items(): # 遍历tracklet中所有的instance
            if cur_frame_id - tracklet.frame_ids[-1] > self.num_dead_frames: # 当前帧序号与instance最后出现的帧序号大于阈值
                dead_ids.append(instance_id)
        for dead_id in dead_ids:
            del self.tracklets[dead_id] # 取消对instance的标号，不参与reid
            self.num_tracklets -= 1
