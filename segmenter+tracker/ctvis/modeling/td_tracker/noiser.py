...
class Noiser:
    ...
    def match_embds(self, ref_embds, cur_embds):
        ...
        assert match_metric in ["bisoftmax","cosine"], "match_metric wrong"
        ...
        if match_metric == "bisoftmax":
            similarity = torch.mm(cur_embds, ref_embds.t())
            d2t_scores = similarity.softmax(dim=1)
            t2d_scores = similarity.softmax(dim=0)
            cos_sim = (d2t_scores + t2d_scores) / 2
