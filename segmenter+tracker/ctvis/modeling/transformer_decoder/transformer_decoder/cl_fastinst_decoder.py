...
class CLFastInstDecoder(nn.Module):
    ...
    def reset_reference(self):
        del self.last_query
        del self.last_embed
        del self.last_pixel_feature
        #del self.last_middle_pixel_feature
        del self.last_e3
        self.last_query = None
        self.last_embed = None
        self.last_pixel_feature = None
        #self.last_middle_pixel_feature = None
        self.last_e3 = None
        return
    ...
    def forward(self, x, mask_features, targets=None,isfirst=False): # isfirst=True 视频第一帧
        ...
        #pixel_features = pixel_features.flatten(2).permute(2, 0, 1) ##
        ...
        outputs_class, outputs_mask, attn_mask, _, _ ,outputs_query, outputs_embed= self.forward_prediction_heads(
        ...
        for i in range(self.num_layers):
            ...
            if i < self.num_layers - 1:
                ...
                outputs_class, outputs_mask, _, matching_indices, gt_attn_mask,outputs_query, outputs_embed = self.forward_prediction_heads(
        ...
        self.last_query = predictions_query[-1].detach()
        self.last_embed = predictions_embed[-1].detach()
        self.last_pixel_feature = pixel_feature_memory[-1].detach()
        ...
        if self.training:
            for i in range(self.num_layers):
                ...
                outputs_class, outputs_mask, _, _, _,outputs_query, outputs_embed = self.forward_prediction_heads(
    ...
    def forward_prediction_heads(self, query_features, pixel_features, pixel_feature_size, idx_layer,
        ...
        return outputs_class, outputs_mask, attn_mask, matching_indices, gt_attn_mask,decoder_query_features, reid_embed
